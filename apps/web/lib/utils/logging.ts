...
export function redactSensitiveInfo(message: string): string {
  ...
  const apiKeyPatterns = [
    ...
    // Also match URLs with sensitive parameters (e.g., ?token=, ?key=, ?secret=)
    /[?&](token|key|secret|password|api_key|api_token|access_token|refresh_token)=([^&\s"'<>]+)/gi,
  ]
  ...
  apiKeyPatterns.forEach((pattern) => {
    redacted = redacted.replace(pattern, (match, key) => {
      ...
      if (match.includes('github.com')) {
        const sensitivePart = key // key is the captured token part
        const redactedKey =
          sensitivePart.length > 8
            ? `${sensitivePart.substring(0, 4)}${'*'.repeat(Math.max(8, sensitivePart.length - 8))}${sensitivePart.substring(sensitivePart.length - 4)}`
            : '*'.repeat(sensitivePart.length)
        // Replace the token in the URL while preserving the structure
        return match.replace(sensitivePart, redactedKey)
      }
  ...
  redacted = redacted.replace(
    /"(teamId|projectId|password|secret|token|apiKey)"[\s:]*"([^"]+)"/gi,
    (match, fieldName) => {
  ...
  redacted = redacted.replace(
    /([A-Z_]*(?:KEY|TOKEN|SECRET|PASSWORD|TEAM_ID|PROJECT_ID|CLIENT_ID|CLIENT_SECRET)[A-Z_]*)[=\s:]*["']?([a-zA-Z0-9_-]{8,})["']?/gi,
    (match, varName, value) => {
      const redactedValue =
        value.length > 8
          ? `${value.substring(0, 4)}${'*'.repeat(Math.max(8, value.length - 8))}${value.substring(value.length - 4)}`
