...
export async function executeQwenInSandbox(
  ...
  await logger.info('Using Qwen model')
...
if response.status_code == 200:
  ...
  await safeExecutor.executeSafe('sh', ['-c', `echo '${pythonScript}' > /tmp/qwen_ollama.py`])

  const safeInstruction = instruction.replaceAll('"', String.raw`\"`)
  const result = await safeExecutor.executeSafe('python3', ['/tmp/qwen_ollama.py', safeInstruction])
