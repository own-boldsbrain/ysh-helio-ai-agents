...
for (const file of filesToCheck) {
  ...
  try {
    ...
    const safeUsage = content.includes('parseJsonResponse') || 
...
try {
  const result = execSync('grep -r "await response.json()" --include="*.ts" --include="*.tsx" . --exclude-dir=node_modules --exclude-dir=.next --exclude-dir=.turbo --exclude="test-json-parsing.ts" --exclude="test-json-parsing.test.ts"', {
  ...
  for (const line of lines) {
    if (line && !line.includes('parseJsonResponse') && !line.includes('safeJson') && !line.includes('getErrorMessage')) {
...
try {
  ...
  if (utilContent.includes('parseJsonResponse') && 
